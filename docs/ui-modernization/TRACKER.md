...
| A11Y-02  | Consolidate overlay focus, Escape, inert, and focus-return behavior     | 2     | P1       | DONE | AUTH-01; BASE-01 where protected                           | Shared overlay foundation plus five child families        | Commit `ff6e06f`: `useOverlayFocus` centralizes topmost-overlay Tab/Shift+Tab containment, Escape/pending behavior, focus return, and nested-safe body scroll lock. ConfirmDialog is the reference adopter; technical/rendered verification passed. Manual user review deferred by explicit authorization for MEGA RUN A v2 |
