...
| BASE-01  | Authenticated protected-screen rendered baseline                        | 0     | P0       | READY   | Local Supabase and approved local administrator | Representative protected routes at required viewports | Local authenticated baseline explicitly authorized; still requires rendered evidence |
...
| INT-01 | Resolve dead or misleading controls | 5 | P2 | READY | AUTH-01 | Dashboard `Xem tất cả`; login options | Remove dead Remember Login, add Show Password, retain default Supabase session behavior; see DEC-UI-015 |
