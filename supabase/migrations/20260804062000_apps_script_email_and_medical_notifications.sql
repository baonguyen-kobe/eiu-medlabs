...
begin
  ...
  set schedule_date = target_schedule_date, updated_at = now()
  ...
  if target_schedule_date is distinct from before_row.schedule_date then
    ...
    select
      ...
      concat(
        '[MedLabs Calendar] ',
        case when room_type_code_value = 'basic_medical'
          then 'Điều chỉnh lịch Y cơ sở · '
          else 'Đổi ngày học · ' end,
...
begin
  ...
  select
    case when room_type_code_value = 'basic_medical'
      then 'class_schedule_basic_medical_created'
      else 'class_schedule_created' end,
    ...
    concat(
      '[MedLabs Calendar] ',
      case when room_type_code_value = 'basic_medical'
        then 'Tạo lịch Y cơ sở · '
        else 'Lịch lớp mới · ' end,
...
begin
  ...
  select case when room_type_code_value = 'basic_medical'
      then 'class_schedule_basic_medical_import_summary'
      else 'class_schedule_import_summary' end,
    recipient.id, recipient.email,
    concat(
      case when room_type_code_value = 'basic_medical'
        then 'class_schedule_basic_medical_import_summary:'
    ...
    concat(
      '[MedLabs Calendar] ',
      case when room_type_code_value = 'basic_medical'
        then 'Import lịch Y cơ sở · '
        else 'Tổng hợp import · ' end,
