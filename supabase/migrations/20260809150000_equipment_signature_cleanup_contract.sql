...
begin
  ...
  if current_user not in ('service_role','postgres') then raise exception 'EQUIPMENT_SIGNATURE_CLEANUP_SERVICE_REQUIRED' using errcode = '42501'; end if;
  return query with candidates as (
    ...
    where o.state in ('pending','rejected')
      ...
      and (o.cleanup_state in ('none','retry') or (o.cleanup_state='claimed' and o.cleanup_claimed_at < target_claimed_before))
  ...
  ) select id,request_id,phase,object_path,state,cleanup_claim_token from claimed;
end;
$$;
