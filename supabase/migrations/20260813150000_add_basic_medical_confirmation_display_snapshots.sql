-- Forward-only display snapshots for confirmation evidence. Legacy rows remain null.
...
alter table public.basic_medical_session_confirmations
  ...
  add column room_code_snapshot text,
  add column building_code_snapshot text,
  add column room_name_snapshot text,
  add column teaching_lecturer_name_snapshot text,
...
declare
  schedule_row record;
begin
  -- Historical rows predate these columns. Only future confirmation inserts
  -- obtain display values, exactly once, from the state visible at signing.
