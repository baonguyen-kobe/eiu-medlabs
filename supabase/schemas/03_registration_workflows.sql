...
create or replace function public.mark_equipment_signature_cleanup_compensation(target_operation_id uuid)
returns void language plpgsql security definer set search_path = '' as $$
declare operation_row public.equipment_signature_operations;
begin
  if target_operation_id is null then raise exception 'EQUIPMENT_SIGNATURE_CLEANUP_INPUT_INVALID' using errcode = '22023'; end if;
  select * into operation_row from public.equipment_signature_operations where id = target_operation_id for update;
  if operation_row.id is null then raise exception 'EQUIPMENT_SIGNATURE_OPERATION_NOT_FOUND' using errcode = 'P0002'; end if;
...
begin
  ...
  return query with candidates as (
      ...
      and (
        ...
        or (
          ...
          and (
            ...
            or (o.state='rejected' and coalesce(o.finalized_at,o.created_at) < target_rejected_before)
          )
        )
      )
      and (
    ...
    order by (o.cleanup_compensation_required_at is null), o.cleanup_compensation_required_at, o.last_reserved_at
...
revoke execute on function public.mark_equipment_signature_cleanup_compensation(uuid) from public, anon, authenticated;
...
grant execute on function public.mark_equipment_signature_cleanup_compensation(uuid) to service_role;
