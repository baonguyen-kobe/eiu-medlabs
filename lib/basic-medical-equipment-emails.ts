...
export async function enqueueBasicMedicalEquipmentDamageEmails({
  ...
  const { data: confirmation, error: confirmationError } = await supabase
    ...
    .select(
      "signed_at,class_schedule_id_snapshot,signer:profiles!basic_medical_session_confirmations_signer_id_fkey(full_name)",
    )
  ...
  const notifications = recipients
    ...
    .map((recipient) => ({
      ...
      subject: basicMedicalDamageEmailSubject(roomCode, roomName),
      payload: {
